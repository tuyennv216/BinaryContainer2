...
{
	public class TypeSByte : TypeOperatorBase
	{
		public TypeSByte()
		{
			Raw = typeof(SByte);
		...
		public override void Write(DataContainer container, object? data, RefPool refPool)
		{
			...
			if (value != 0)
			{
				var byteData = unchecked((byte)(sbyte)data);
				container.Items.Add(byteData);
			}
		}
		...
		public override object? Read(DataContainer container, RefPool refPool)
		{
			...
			if (isdefault == true) return (SByte)0; // Trả về 0 với kiểu SByte

			var bytes = container.ReadItems(1); // SByte là 1 byte
